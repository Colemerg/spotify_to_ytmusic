...
def build_results(tracks, album=None):
    ...
    for track in tracks:
        if "track" in track:
            track = track["track"]
        if not track or track.get("duration_ms", 0) == 0:
            continue
        # Eğer "album" anahtarı eksikse varsayılan olarak "Bilinmeyen Albüm" kullanılıyor.
        album_name = album if album is not None else track.get("album", {}).get("name", "Bilinmeyen Albüm")
        # "artists" anahtarını dict.get() ile kontrol edip, varsa sanatçı adlarını alıyoruz.
        artist_names = " ".join([artist.get("name", "Bilinmeyen Sanatçı") for artist in track.get("artists", [])])
        results.append(
            {
                "artist": artist_names,
